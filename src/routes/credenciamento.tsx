...
import {
  ...
  CREDENCIAMENTO_DESCRIPTION,
  CREDENCIAMENTO_FAQS,
  CREDENCIAMENTO_HOWTO_STEPS,
  CREDENCIAMENTO_KEYWORDS,
  CREDENCIAMENTO_PATH,
...
import { CredenciamentoSeoBlock } from "@/components/credenciamento/CredenciamentoSeoBlock";

const PATH = CREDENCIAMENTO_PATH;
const TITLE = CREDENCIAMENTO_TITLE;
const DESCRIPTION = CREDENCIAMENTO_DESCRIPTION;

const CREDENCIAMENTO_JSON_LD = (() => {
  const schemas = buildCredenciamentoStructuredData({
    title: TITLE,
    description: DESCRIPTION,
    abstract: CREDENCIAMENTO_ABSTRACT,
    path: PATH,
    faqs: CREDENCIAMENTO_FAQS,
    howToSteps: CREDENCIAMENTO_HOWTO_STEPS,
  });
  return [
    schemas.webPage,
    schemas.breadcrumb,
    schemas.service,
    schemas.howTo,
    ...(schemas.faqPage ? [schemas.faqPage] : []),
  ];
})();
...
export const Route = createFileRoute("/credenciamento")({
  ...
  head: () =>
    buildSeoHead({
      ...
      description: DESCRIPTION,
      abstract: CREDENCIAMENTO_ABSTRACT,
      keywords: CREDENCIAMENTO_KEYWORDS,
      ogType: "website",
      jsonLd: CREDENCIAMENTO_JSON_LD,
    }),
...
function CredenciamentoPage() {
  ...
  useEffect(() => {
    if (!isFinal || answeredCount < totalSteps) return;
    trackCredenciamentoComplete({
      score,
      oportunidades,
      tier: tier.label,
    });
  }, [isFinal, answeredCount, totalSteps, score, oportunidades, tier.label]);

  function select(v: Value) {
    const opt = currentQ.options.find((o) => o.value === v);
    trackCredenciamentoStep({
      questionId: currentQ.id,
      questionLabel: currentQ.title,
      answer: String(v),
      stepIndex: step + 1,
      totalSteps,
    });
