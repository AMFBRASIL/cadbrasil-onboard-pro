...
export const criarCadastro = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: rawInput }): Promise<CriarCadastroResult> => {
    ...
    try {
      ...
      // Tracking de origem (Google/Bing/Meta) — não bloqueante, fora da transação.
      await insertTrackingSessao(idCliente, idUsuario, data.tracking ?? {});

      // E-mail de início de processo — em background (não bloqueia a resposta ao cliente).
      void import("./email")
    ...
    // ---- helpers internos (mantêm compatibilidade com variações de schema) ----
    ...
    async function insertTrackingSessao(
      ...
      try {
        ...
        // Tracking nunca deve bloquear o cadastro já concluído.
